...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self): 
        ...
        start_position_in_index_file, number_of_postings, len_in_bytes_of_postings, len_in_bytes_of_tf = self.postings_dict[curr_term]
    ...
    def get_postings_list(self, term):
        ...
        """
        # TODO Done
        start_position_in_index_file, number_of_postings, len_in_bytes_of_postings, len_in_bytes_of_tf = self.postings_dict[term]
    ...
    def append(self, term, postings_list, tf_list):
        ...
        for key in postings_list:
            for value in tf_list:
                dct[key] = value
                tf_list.remove(value)
                break

        if self.terms != []:
            (start_position_in_index_file, number_of_postings_in_list, length_in_bytes_of_postings_list, 
                length_in_bytes_of_tf_list) = self.postings_dict[self.terms[-1]]

            self.postings_dict[term] = (start_position_in_index_file + length_in_bytes_of_postings_list
                + length_in_bytes_of_tf_list, 
                len(postings_list), len(encoded_postings_list), len(encoded_tf_list))
    
            result = sorted_merge_posts_and_tfs([(k, v) for k, v in dct.items()], 
                [(k, v) for k, v in self.doc_length.items()])
            
            self.doc_length =  dict(i for i in result)
            
        else:
            self.postings_dict[term] = (0, len(postings_list), len(encoded_postings_list), 
                len(encoded_tf_list))
